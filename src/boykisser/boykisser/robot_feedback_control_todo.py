...
class ControlStrategy(Node):
    ...
    def diff_drive_init(self, left_wheel_v, right_wheel_v, duration=50):
    ...
    def inter_point_diff_drive(self, ):
        if(self.robot_current_pose is not None):
            ...
            # TODO Estimate the distance (self.D) between current pose (self.robot_current_pose) #
            # and reference pose (self.refPose) 
            self.D = math.sqrt(math.pow(self.refPose[0] - self.robot_current_pose_real[0],2) + math.pow(self.refPose[1] - self.robot_current_pose_real[1],2))
            ...
            self.D = np.sqrt((self.robot_current_pose_real[0]-self.refPose[0])**2 
                             + (self.robot_current_pose_real[1]-self.refPose[1])**2)
            ...
            if self.state == 0:
                d = np.sqrt((self.yT-self.robot_current_pose_real[1])**2 
                            + (self.xT-self.robot_current_pose_real[0])**2)
                if(d < self.dmin):
                    self.state = 1
                self.phiT = np.arctan2(self.yT-self.robot_current_pose_real[1], self.xT-self.robot_current_pose_real[0])
                self.ePhi = self.phiT - self.robot_current_pose_real[2]
            else:
                self.ePhi = self.refPose[2] - self.robot_current_pose_real[2]
            ...
            # TODO Estimate orientation residual (self.ePhi) 
            #self.ePhi = math.atan2((self.refPose[1] - self.robot_current_pose_real[1]),(self.refPose[0] - self.robot_current_pose_real[0]))
            #print("Distance to the goal: theta_ref={} theta_act={}".format(self.refPose[2], self.robot_current_pose_real[2]))
            #self.ePhi = self.refPose[2] - self.robot_current_pose_real[2]
           
            v = self.k_p*self.D
            w = self.k_w*self.ePhi
            print("Distance to the goal: {} v={} w={}".format(self.D, v, w))
